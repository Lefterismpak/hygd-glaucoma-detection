...
On the correct predictions, the heatmap concentrates on the optic disc / peripapillary region in most cases — the clinically relevant area for glaucomatous cupping, which is a reassuring sanity check (the model isn't keying off unrelated artifacts). The per-error clinical write-up (why each wrong prediction plausibly happened) is intentionally left as a TODO in `notebooks/04_explainability.ipynb` — that judgment benefits from real ophthalmology input, not an auto-generated guess.
