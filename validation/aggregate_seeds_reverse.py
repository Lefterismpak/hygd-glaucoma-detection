"""Aggregate five current patient-aware reverse runs as adaptive evidence only."""
...
INPUT_PATTERN = "reverse_seed{seed}.json"
DEFAULT_OUTPUT = f"{NAMESPACE}/seed_robustness_reverse_patient_cluster_v2.json"
EXPECTED_RUN_KEYS = {
    "_evidence_status",
    ...
    "seed",
    "held_out_dataset",
    "adaptive_rimone_image_level_auroc_TTA",
    "rimone_patient_cluster_uncertainty",
    "vcdr_weight",
    "vcdr_supervised_on",
    "sources",
}
EXPECTED_VCDR_SOURCE = (
    "PAPILA only (dataset-provided two-grader contour-derived VCDR; "
    "historical auxiliary-label rule); HYGD masked out"
)
...
def _load_run(path: Path) -> tuple[dict, bytes]:
    ...
    try:
        ...
        raise ValueError(f"Invalid reverse-seed JSON: {path.name}") from error
...
def summarize_runs(paths: Sequence[Path]) -> dict:
    if len(paths) != 5:
        raise ValueError("Exactly five reverse seed artifacts are required")
    runs = []
    mapping_hashes = set()
    source_records = []
    for path in paths:
        payload, source_digest = _load_run(path)
        require_exact_keys(payload, EXPECTED_RUN_KEYS, f"Reverse artifact {path.name}")
        validate_evidence_status(
            payload["_evidence_status"], f"Reverse artifact {path.name}"
        )
        if (
            payload["attempt"] != "B_reverse_multitask_VCDR"
            or type(payload["vcdr_weight"]) is not float
            or payload["vcdr_weight"] != 0.5
            or payload["held_out_dataset"] != "RIMONE"
            or payload["sources"] != ["HYGD", "PAPILA"]
        ...
        auroc = require_probability(
            payload["adaptive_rimone_image_level_auroc_TTA"],
            f"Reverse AUROC: {path.name}",
        )
        ...
        validate_seed_filename(
            path, pattern=INPUT_PATTERN, seed=seed, context="Reverse artifact"
        )
        if not is_lowercase_sha256(mapping_hash):
            raise ValueError(f"Invalid subject-map binding: {path.name}")
        validate_patient_cluster_uncertainty(
            payload["rimone_patient_cluster_uncertainty"],
            top_level_auroc=auroc,
            context=f"Reverse patient-cluster uncertainty: {path.name}",
        )
    ...
    if [seed for seed, _ in runs] != list(range(5)):
        raise ValueError("Reverse seed artifacts must contain unique seeds 0..4")
    if len(mapping_hashes) != 1:
        raise ValueError("Reverse seed artifacts must share one subject-map binding")
    ...
    return {
        ...
        "metric": "adaptive_RIMONE_image_level_AUROC_TTA",
        ...
        "subject_independence": "operator_asserted_hash_bound_needs_independent_proof",
        "n_seeds": 5,
        "seeds": list(range(5)),
        ...
        "input_bundle_algorithm": "sha256(HYGD_REVERSE_SEED_BUNDLE_V1 newline + canonical JSON seed/file/hash records + newline)",
        "input_bundle_sha256": hashlib.sha256(
            b"HYGD_REVERSE_SEED_BUNDLE_V1\n"
            + json.dumps(
                source_records,
                ensure_ascii=True,
                separators=(",", ":"),
        ...
        "note": "Sample SD across five adaptive runs is not a confidence interval; RIM-ONE subject independence and transportability remain needs-proof.",
    }
